...
# Usage: run from a machine with SSH access to the NAS
#        ./install-optional-tweaks.sh [sysctl|usb|all]
#
...
usage() {
  echo "Usage: NAS_PASS=<pw> $0 [sysctl|usb|all]"
  echo ""
...
for arg in "$@"; do
  case "$arg" in
    ...
    usb)    install_usb ;;
    all)    install_sysctl; install_usb ;;
    *)      usage ;;
  esac
done
