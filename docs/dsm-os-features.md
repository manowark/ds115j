...
| **Watchdog** | (DSM does not feed it either) | ✅ | SoC **Orion Watchdog** at `/dev/watchdog0`, timeout **257 s → 60 s when fed**. **LIVE-VERIFIED 2026-09-21**: after the reboot following the `RuntimeWatchdogSec=60` deploy, `systemd` reports `RuntimeWatchdogUSec=1min`, `/sys/class/watchdog/watchdog0/state` = **`active`**, `timeout` = `60` — i.e. systemd feeds it and a hard kernel hang now reboots the board instead of leaving it dead |
