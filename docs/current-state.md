...
| DSM OS-parity tuning (2026-09-21) | **✅ APPLIED** — `somaxconn=65535`, `accept_ra_defrtr=0` (live), USB autosuspend **off** `-1` (live), SoC Orion watchdog `RuntimeWatchdogSec=60` (armed, effective next boot). RTC power-schedule: **LIVE-TESTED → does NOT work** with our PSU-cut poweroff (armed alarm +3 min did not wake the box); but **RTC calendar survives full power cut** (coin-cell standby rail). See [`dsm-os-features.md`](dsm-os-features.md) |
