...
**Power button (2026-09-21):** `syno-powerbtn.service` + `syno-powerbtn-arm.service` are installed and **armed** (`/etc/syno-powerbtn.conf` → `TRIGGER_BYTES="30"`, `SETTLE_SEC=60`): the front button performs a **clean power-off** (verified live — press → `0x30` → `systemctl poweroff` → `qnap_poweroff_ds115j` cuts PSU). The daemon is read-only and never writes to `/dev/ttyS1`; the arming handshake (DSM rc bytes `echo 4`/`echo 9`) is sent once at boot by the arm unit and it **persists through power-off** (PIC standby rail). Landmines that remain: **never send `'1'`/`'C'`** (ack/power-off channels), and the button needs the *newline-terminated* rc bytes to report — bare LED bytes are not enough. Details + SPI-forensics proof that the button reaches Linux only via the PIC on UART1: [`power-button.md`](power-button.md).
